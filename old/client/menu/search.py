...
class Search(object):
    ...
    def __init__(self, api_port, keyword, type=None):
        ...
        self.file_channel = os.path.join(
            os.path.dirname(__file__), 'channel.result')
        self.file_torrent = os.path.join(
            os.path.dirname(__file__), 'torrent.result')

        (self.results, self.sender) = multiprocessing.Pipe()

    def start_searching(self):
        from client.module.perform_request import request_search
        from client.module.utilities import get_string

        self.search_manager_thread = multiprocessing.Process(
            target=self.search_manager, args=(self, ))
        self.search_manager_thread.start()

        # blocking, wait for the event channel. discarding the 1st message
        # which is events_start
        self.results.recv()
        ...
        while not dialog_progress.iscanceled():
            dialog_progress.update(progress_bar, '%d ' % len(results_channel) + get_string(
                33203), '%d ' % len(results_torrent) + get_string(33204), get_string(33205))
            time.sleep(0.06)
            if progress_bar < 99:
    ...
    def channel_results(self):
        ...
        for result in results_channel:
            ...
            self.addDirectoryItem('[channel] ' + name, 'open-channel&channel-id=%s&name=%s' %
    ...
    def torrent_results(self):
        ...
        for result in results_torrent:
            ...
            self.addDirectoryItem('[%s] ' % result.get('result').get(
                'category') + name, 'torrent-action&info-hash=%s&name=%s' % (info_hash, name), 'recommanded.png', 'DefaultMovies.png')
    ...
    def search_manager(self, parent):
        response = requests.get('http://localhost:%d/events' %
                                self.api_port, stream=True)
