...
class Torrent(object):
    ...
    def recommanded_torrents(self):
        limit = 9
        from client.module.perform_request import request_random_torrents
    ...
    def open_channel(self, channel_id, name):
        from client.module.perform_request import request_channel_torrents
        torrent_list = request_channel_torrents(8085, channel_id)

        self.addDirectoryItem(33001, 'subscribe&channel-id=%s&name=%s' %
        ...
        for torrent_info in torrent_list:
            ...
            if name == 'Unknown name' or name == 'Unnamed torrent':
                continue
            info_hash = torrent_info.get('infohash')
            self.addDirectoryItem(name, 'torrent-action&info-hash=%s&name=%s' %
                                  (info_hash, name), 'recommanded.png', 'DefaultMovies.png')
    ...
    def torrent_action(self, info_hash, name):
        ...
        action = xbmcgui.Dialog().select(get_string(
            33004), [get_string(33005), get_string(33006), get_string(33007)])
        if action == 0:
