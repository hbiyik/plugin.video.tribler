...
class Setting(object):
    ...
    def load_from_ui(self):
        self.family_filter = bool(
            xbmcaddon.Addon().getSetting('filter.enable') == 'true')
        self.download_direction = str(
            xbmcaddon.Addon().getSetting('dllocation.path'))
        self.ask_download_settings = bool(
            xbmcaddon.Addon().getSetting('dllocation.ask') == 'true')
        self.anonymous_download = bool(
            xbmcaddon.Addon().getSetting('dlsetting.anony') == 'true')
        self.anonymous_seed = xbmcaddon.Addon().getSetting('dlsetting.encrypt')
        self.watch_folder = bool(
            xbmcaddon.Addon().getSetting('wfolder.enable') == 'true')
        self.watch_folder_dir = str(
            xbmcaddon.Addon().getSetting('wfolder.path'))
        self.developer_mode = bool(
            xbmcaddon.Addon().getSetting('devmode.enable') == 'true')
        self.port = int(xbmcaddon.Addon().getSetting('firewall.port'))
        self.lt_proxy_type = int(xbmcaddon.Addon().getSetting('tproxy.type'))
        self.lt_proxy_server = str(
            xbmcaddon.Addon().getSetting('tproxy.server'))
        self.lt_proxy_port = str(xbmcaddon.Addon().getSetting('tproxy.port'))
        self.lt_proxy_username = str(
            xbmcaddon.Addon().getSetting('tproxy.user'))
        self.lt_proxy_password = str(
            xbmcaddon.Addon().getSetting('tproxy.pass'))
        self.bandwidth_management = bool(
            xbmcaddon.Addon().getSetting('btfeature.utp') == 'true')
        self.bandwidth_connection = int(
            xbmcaddon.Addon().getSetting('btfeature.connect'))
        self.upload_limit = int(xbmcaddon.Addon().getSetting('bwlimit.upload'))
        self.download_limit = int(
            xbmcaddon.Addon().getSetting('bwlimit.download'))
        self.seeding_option = int(
            xbmcaddon.Addon().getSetting('seeding.option'))
        self.seeding_ratio = float(
            xbmcaddon.Addon().getSetting('seeding.ratio'))
        ...
        self.exitnode = bool(xbmcaddon.Addon().getSetting(
            'anony.exitnode') == 'true')
        self.default_hop = int(
            xbmcaddon.Addon().getSetting('proxydl.speed')) + 1
        self.multichain = bool(
            xbmcaddon.Addon().getSetting('multichain.enable'))
    ...
    def load_from_server(self):
        ...
        seeding_modes = {'forever': '0',
                         'time': '1', 'never': '2', 'ratio': '3'}

        xbmcaddon.Addon().setSetting('filter.enable',
                                     TF[settings_data['general']['family_filter']])
        xbmcaddon.Addon().setSetting('dllocation.path', json.dumps(
            settings_data['downloadconfig']['saveas']))
        xbmcaddon.Addon().setSetting('wfolder.enable',    json.dumps(
            TF[settings_data['watch_folder']['enabled']]))
        xbmcaddon.Addon().setSetting('wfolder.path',     json.dumps(
            settings_data['watch_folder']['watch_folder_dir']))
        xbmcaddon.Addon().setSetting('firewall.port',
                                     str(settings_data['general']['minport']))
        xbmcaddon.Addon().setSetting('tproxy.type',     str(
            settings_data['libtorrent']['lt_proxytype']))
        if settings_data['libtorrent']['lt_proxyserver']:
            xbmcaddon.Addon().setSetting('tproxy.server',
                                         settings_data['libtorrent']['lt_proxyserver'][0])
            xbmcaddon.Addon().setSetting('tproxy.port', str(
                settings_data['libtorrent']['lt_proxyserver'][1]))
        else:
            xbmcaddon.Addon().setSetting('tproxy.server', '')
            xbmcaddon.Addon().setSetting('tproxy.port', '')
        if settings_data['libtorrent']['lt_proxyauth']:
            xbmcaddon.Addon().setSetting(
                'tproxy.user', settings_data['libtorrent']['lt_proxyauth'][0])
            xbmcaddon.Addon().setSetting(
                'tproxy.pass', settings_data['libtorrent']['lt_proxyauth'][1])
        else:
            xbmcaddon.Addon().setSetting('tproxy.user', '')
            xbmcaddon.Addon().setSetting('tproxy.pass', '')
        xbmcaddon.Addon().setSetting('btfeature.utp',
                                     TF[settings_data['libtorrent']['utp']])
        if settings_data['libtorrent']['max_connections_download'] == -1:
            xbmcaddon.Addon().setSetting('btfeature.connect', '0')
        else:
            xbmcaddon.Addon().setSetting('btfeature.connect', str(
                settings_data['libtorrent']['max_connections_download']))
        xbmcaddon.Addon().setSetting('bwlimit.upload', str(
            settings_data['Tribler']['maxuploadrate']))
        xbmcaddon.Addon().setSetting('bwlimit.download', str(
            settings_data['Tribler']['maxdownloadrate']))
        xbmcaddon.Addon().setSetting('seeding.option',
                                     seeding_modes[settings_data['downloadconfig']['seeding_mode']])
        xbmcaddon.Addon().setSetting('seeding.ratio', str(
            settings_data['downloadconfig']['seeding_ratio']))
        xbmcaddon.Addon().setSetting('seeding.hour', str(
            int(settings_data['downloadconfig']['seeding_time']) / 60))
        xbmcaddon.Addon().setSetting('seeding.min', str(
            int(settings_data['downloadconfig']['seeding_time']) % 60))
        xbmcaddon.Addon().setSetting('anony.exitnode',
                                     TF[settings_data['tunnel_community']['exitnode_enabled']])
        xbmcaddon.Addon().setSetting('proxydl.speed', str(
            settings_data['Tribler']['default_number_hops'] - 1))
        xbmcaddon.Addon().setSetting('multichain.enable',
                                     TF[settings_data['multichain']['enabled']])
